...
def extract_region(padded_image: np.ndarray, center_row: int, center_col: int, window_size: int) -> np.ndarray:
    ...
    # ToDo: Use slicing.
    return padded_image[center_row - window_size // 2:center_row + window_size // 2 + 1,
           center_col - window_size // 2:center_col + window_size // 2 + 1]
...
def erode_binary(image: np.ndarray, structuring_element: np.ndarray) -> np.ndarray:
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            region = extract_region(padded_image, i + se_size // 2, j + se_size // 2, se_size)
